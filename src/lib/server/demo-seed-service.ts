...
/** Extract storage paths from an application's `files` JSON column. */
export function collectStoredFilePaths(files: Json): string[] {
...
export async function seedDemoApplicant(
  supabase: ReturnType<typeof getSupabaseAdminClient>,
  profile: DemoProfile,
...
/* -------------------------------------------------------------------------- */
/*  Ensure OCR config for ID document field                                   */
/* -------------------------------------------------------------------------- */

export async function ensureIdDocOcrConfig(
  supabase: ReturnType<typeof getSupabaseAdminClient>,
): Promise<boolean> {
