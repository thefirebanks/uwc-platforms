...
function describePatchValidationError(
  error: z.ZodError<z.infer<typeof patchSchema>>,
) {
  ...
  return withErrorHandling(
    async () => {
      ...
      const stageIdentifierParsed =
        stageIdentifierSchema.safeParse(rawStageCode);
      ...
      if (automationsError) {
        throw new AppError({
          ...
          userMessage:
            "No se pudieron cargar las automatizaciones de la etapa.",
          status: 500,
      ...
      const rawFields = ((fieldsData as StageFieldRow[] | null) ?? []).map(
        (row) => ({
          ...row,
          section_id:
            ((row as Record<string, unknown>).section_id as string | null) ??
            null,
        }),
      );
      const fields =
      ...
      const parsedAdminConfig = parseStageAdminConfig(
        (
          templateData as
            | (Pick<StageTemplateRow, "admin_config"> &
                Pick<
                  StageTemplateRow,
                  "stage_label" | "due_at" | "ocr_prompt_template"
                >)
            | null
        )?.admin_config ?? null,
      );
    ...
    async (requestId) => {
      ...
      const incomingSettings = parsed.data.settings
        ? {
            ...
            previousStageRequirement:
              parsed.data.settings.previousStageRequirement,
        ...
        : {
            ...
            eligibilityRubric:
              parsedExistingAdminConfig.eligibilityRubric ?? null,
            rubricBlueprintV1:
      ...
      if (new Set(automationKeys).size !== automationKeys.length) {
        throw new AppError({
          ...
          userMessage:
            "No puedes repetir automatizaciones para el mismo evento.",
          status: 400,
        });
      }
      ...
      if (parsed.data.sections) {
        const incomingSectionKeys = new Set(
          parsed.data.sections.map((s) => s.sectionKey.trim()),
        );
      ...
      if (deleteAutomationsResult.error) {
        throw new AppError({
          ...
          userMessage:
            "No se pudieron guardar las automatizaciones de la etapa.",
          status: 500,
      ...
      const {
        inserts: automationRowsToInsert,
        updates: automationRowsToUpdate,
      ...
      if (updateAutomationsResult.error || insertAutomationsResult.error) {
        throw new AppError({
          ...
          status: 500,
          details:
            updateAutomationsResult.error ?? insertAutomationsResult.error,
        });
      }
      ...
      const { data: savedTemplateData, error: savedTemplateError } =
        await supabase
          .from("cycle_stage_templates")
          .update({
      ...
      const savedTemplate =
        (savedTemplateData as Pick<
          StageTemplateRow,
          "stage_label" | "due_at" | "ocr_prompt_template" | "admin_config"
        > | null) ?? null;
      ...
      return NextResponse.json({
        ...
        settings: {
          ...
          openDate:
            (stageCode === "documents" || stageCode === "exam_placeholder"
              ? savedCycleOpenDate
              : (savedAdminConfig.openDate ?? incomingSettings.openDate)) ??
            null,
          closeDate:
            (stageCode === "documents" || stageCode === "exam_placeholder"
              ? savedCycleCloseDate
              : (savedAdminConfig.closeDate ?? incomingSettings.closeDate)) ??
            null,
          previousStageRequirement:
